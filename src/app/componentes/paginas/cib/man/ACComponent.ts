...
export abstract class ACComponent {
    ...
    constructor(protected cibService: CibService,
    ...
    guardar() {
        ...
        if (detallesNuevos.length > 0) {
            this.cibService.insertarDetalles(this.url, detallesNuevos).then((data: any) => {
                this.loading = false;
        ...
        if (detallesModificar.length > 0) {
            this.cibService.actualizarDetalles(this.url, detallesModificar).then((data: any) => {
                this.loading = false;
                if (data.respuestaCodigo === -1) {
                    this.msgsActualizar = [];
    ...
    eliminar(listaEliminar) {
        ...
        this.cibService.actualizarDetalles(this.url, listaEliminarBase).then((data: any) => {
            this.loading = false;
            if (data.respuestaCodigo === -1) {
                this.msgsActualizar = [];
