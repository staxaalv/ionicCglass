...
export abstract class ACComponent {
    ...
    constructor(protected ageService: AgeService,
    ...
    guardar() {
        ...
        if (detallesNuevos.length > 0) {
            this.ageService.insertarDetalles(this.url, detallesNuevos).then((data: any) => {
                this.loading = false;
        ...
        if (detallesModificar.length > 0) {
            this.ageService.actualizarDetalles(this.url, detallesModificar).then((data: any) => {
                this.loading = false;
                if (data.respuestaCodigo === -1) {
                    this.msgsActualizar = [];
    ...
    eliminar(listaEliminar) {
        ...
        this.ageService.actualizarDetalles(this.url, listaEliminarBase).then((data: any) => {
            this.loading = false;
            if (data.respuestaCodigo === -1) {
                this.msgsActualizar = [];
